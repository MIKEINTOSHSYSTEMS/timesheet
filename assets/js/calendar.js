    ...
    function updateAllDateDisplays() {
        document.querySelectorAll('[data-date]').forEach(element => {
            ...
            if (document.body.classList.contains('ethiopian-calendar')) {
                // Convert to Ethiopian if not already
                if (dateStr.match(/^\d{4}-\d{2}-\d{2}$/)) {
                    const ethDate = convertToEthiopian(dateStr);
                    element.textContent = ethDate.date;
                } else {
                    // Assume already Ethiopian
                    element.textContent = dateStr;
                }
                element.classList.add('ethiopian-text');
            } else {
                // Convert to Gregorian if needed
                if (dateStr.match(/^\d{4}-\d{2}-\d{2}$/) && !dateStr.startsWith('19') && !dateStr.startsWith('20')) {
                    const gcDate = convertToGregorian(dateStr);
                    element.textContent = gcDate.date;
                } else {
                    // Assume already Gregorian
                    element.textContent = dateStr;
                }
